...
async function downloadBinary() {
    ...
    if (!info)
        throw new Error(`No SWA client binary for ${platform}`);
    const buildDir = join(CACHE_DIR, meta.buildId);
    const binaryName = process.platform === "win32" ? "StaticSitesClient.exe" : "StaticSitesClient";
    ...
    if (!binResp.ok)
        throw new Error(`Failed to download SWA client: ${binResp.status}`);
    const buffer = Buffer.from(await binResp.arrayBuffer());
    await writeFile(binaryPath, buffer);
    await chmod(binaryPath, 0o755);
    // Update cache metadata
    await writeFile(join(CACHE_DIR, "StaticSitesClient.json"), JSON.stringify({ metadata: meta, binary: binaryPath, checksum: info.sha }));
    return binaryPath;
}
export async function ensureSwaClient() {
    return (await getCachedBinary()) ?? (await downloadBinary());
}
export async function deploySwaContent(apiToken, outputDir) {
...
//# sourceMappingURL=swa-client.js.map