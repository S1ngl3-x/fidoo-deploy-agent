...
export async function configureEasyAuth(
  ...
  if (!existingSecretNames.includes("portal-client-secret")) {
    ...
    const allSecrets: { name: string; value: string }[] = [];
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (poll.ok) {
        const d = await poll.json() as { properties: { provisioningState: string } };
        if (d.properties.provisioningState === "Succeeded") break;
      }
    }
  }
